...
void BSTreeLevelOrder(BSTree t)
{
    ...
    Queue newQ;
    newQ = newQueue(); //initialize queue
    QueueJoin(newQ,t); //add T's root node to the queue
    
//printf("show q:");showQueue(newQ);
    //int isEmpty = QueueIsEmpty(newQ);
    while(!QueueIsEmpty(newQ)){

        Item node = QueueLeave(newQ);
        showBSTreeNode(node);
        if(node->left != NULL)
            QueueJoin(newQ,node->left);
        if(node->right != NULL)
            QueueJoin(newQ,node->right);
//printf("show q:");showQueue(newQ);
